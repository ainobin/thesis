...
\author{
    ...
    \small Student ID: [Your Student ID]\\[6pt]
...
any other degree or diploma at this or any other university.
...
Special thanks to my colleagues and friends who assisted with data collection and offered encouragement during challenging phases of the project.
...
The initial dataset consisted of 30 audio samples (15 per grade). Waveform-level augmentation (pitch shift, time stretch, additive noise) was applied to expand the training set 5-fold to 100 samples. Despite this, the CNN exhibited severe overfitting: 100\% training accuracy versus 40\% test accuracy, predicting all samples as Grade B.
...
Bootstrap 95\% confidence intervals (accuracy: 0.00--0.80) quantitatively confirm that the 5-sample test set is too small for reliable evaluation.
...
Brick manufacturing is a cornerstone of the construction industry, and ensuring consistent quality is critical for structural safety and longevity. Traditional quality assessment methods rely on visual inspection and mechanical testing, both of which have significant limitations. Visual inspection can only detect surface-level defects such as cracks, discoloration, or dimensional irregularities. Mechanical testing, while more thorough, is destructive, time-consuming, and impractical for 100\% production-line inspection.

Non-destructive testing (NDT) offers an alternative paradigm: assessing material properties without damaging the sample. Acoustic NDT---particularly impact-echo testing---has been used in civil engineering to detect internal voids, delamination, and density variations in concrete and ceramics. When a brick is struck, the resulting acoustic signal carries information about its internal structure, density, and elastic modulus. Grade A bricks produce different acoustic signatures than Grade B bricks due to differences in material density, firing uniformity, and internal flaws.

Recent advances in deep learning, particularly convolutional neural networks (CNNs), have demonstrated remarkable success in audio classification tasks. By converting raw audio into Mel-spectrograms (visual representations of frequency content over time), CNNs can learn complex acoustic patterns that correlate with material properties.

\section{Problem Statement}

Manual brick grading is subjective, inconsistent, and does not scale to industrial production volumes. Existing automated systems primarily rely on computer vision, which cannot detect internal defects such as under-burnt cores, internal voids, or density variations that affect structural integrity. An acoustic NDT system powered by deep learning could bridge this gap by analyzing internal material properties through sound.

However, deep learning models require large datasets to generalize. The primary challenge is building a system that can learn meaningful acoustic features from a limited number of labeled samples.
...
\begin{itemize}
    ...
    \item \textbf{Impact-echo testing:} A mechanical impact generates stress waves that reflect off internal boundaries and defects.
\end{itemize}

Impact-echo is particularly relevant to brick assessment because it is low-cost, fast, and sensitive to internal density variations. When a brick is struck, the resulting acoustic signal contains frequency components determined by the brick's geometry, density, and elastic modulus. Defects such as voids, cracks, or non-uniform firing alter these frequency components, producing a measurably different acoustic signature.
...
A spectrogram is the visual representation of the STFT, with time on the x-axis, frequency on the y-axis, and color indicating magnitude. The Mel-spectrogram applies a nonlinear frequency scaling (the Mel scale) that approximates human auditory perception. The Mel scale is linear below 1 kHz and logarithmic above, allocating more resolution to lower frequencies where acoustic discrimination is most sensitive.
...
When applied to Mel-spectrograms, CNNs treat the time-frequency representation as a 2D image. Early convolutional layers learn low-level patterns such as spectral edges and harmonic structures. Deeper layers combine these into higher-level features corresponding to acoustic events and material properties.
...
To mitigate the limited dataset size, four waveform-level augmentations are applied to the training set:
\begin{itemize}
    ...
    \item \textbf{Time stretch (1.1×):} Stretches the audio by 10\% using \texttt{librosa.effects.time\_stretch}.
    \item \textbf{Additive Gaussian noise:} Adds random noise with \(\sigma = 0.005\) to simulate environmental variability.
\end{itemize}

Each original training sample produces 1 original + 4 augmented versions, expanding the training set 5-fold. Augmentation is applied at the waveform level before Mel-spectrogram conversion, ensuring the augmentations affect all stages of the DSP pipeline naturally. Validation and test sets are not augmented to avoid data leakage.

\section{Dataset Splitting}

A stratified 70/15/15 train/val/test split is performed by file index before preprocessing (not on precomputed features). This ensures that all augmented versions of a given original sample remain confined to the training set. The stratification preserves the class balance in each split. The split uses \texttt{sklearn.model\_selection.train\_test\_split} with \texttt{random\_state=42}.
...
\begin{itemize}
    \item \textbf{Convolutional blocks:} Three convolutional layers with increasing filter counts (32, 64, 128) learn hierarchical features. Batch normalization stabilizes training. Max pooling after the first two blocks reduces spatial dimensions by a factor of 4 overall.
...
\begin{tabular}{lcccc}
\toprule
Set & Grade A & Grade B & Total & Augmented \\
\midrule
Training & 10 & 10 & 20 & 100 (5×) \\
Validation & 2 & 3 & 5 & 5 \\
Test & 3 & 2 & 5 & 5 \\
\bottomrule
\end{tabular}
\caption{Dataset splits before and after augmentation}
\label{tab:splits}
\end{table}
...
The model reached 100\% training accuracy by epoch 9 while validation accuracy stagnated at 60\% (3/5 correct) and validation loss increased after epoch 7. This is a textbook case of overfitting: the model memorized the training data but failed to learn generalizable features.
...
Before augmentation, all 16 combinations achieved exactly 60\% validation accuracy, indicating a ceiling effect. After augmentation, 3 combinations reached 80\%, suggesting that augmentation introduced meaningful variety into the training set. The best configuration was: \texttt{learning\_rate=0.0001, dropout=0.3, l2\_reg=0.0001, batch\_size=32}.
...
The ROC and PR curves both show an AUC of 1.000, which is a misleading artifact of the 5-sample test set---any model with a decision threshold that separates the two classes (even randomly) can produce a perfect curve with so few points.
...
The confidence intervals are extremely wide, confirming that the 5-sample test set cannot support reliable performance estimation. The true accuracy could plausibly be anywhere from 0\% to 80\%.
...
The SVM correctly classified 2 of 3 Grade A samples and both Grade B samples, achieving 100\% precision on Grade A.
...
\textbf{Data requirements:} The contrast between CNN and baseline performance establishes a lower bound on the data required for deep learning in acoustic brick classification. With 100 augmented samples, deep learning is not only unnecessary but counterproductive. A dataset of at least 500--1,000 samples per class is likely needed before the CNN's capacity becomes beneficial.
...
The most significant limitation is the dataset size. With only 30 original audio samples (15 per class), the study operates in a ``few-shot-like'' regime where deep learning is known to struggle. The 5-fold augmentation mitigates but does not solve this problem. Key consequences include:

\begin{itemize}
    \item The 5-sample validation and test sets provide insufficient resolution for reliable performance estimation (each sample represents 20\% of the metric).
    \item The hyperparameter search suffers from a ceiling effect; only 20\% resolution means many configurations appear equivalent.
    \item Bootstrap confidence intervals span nearly the entire possible range (0--80\% for accuracy), making any point estimate unreliable.
\end{itemize}
...
Pre-trained audio models such as YAMNet, VGGish, or Wav2Vec2.0 were not evaluated. Transfer learning from models pre-trained on large-scale audio datasets could potentially provide better performance with limited data.
...
\textbf{The evaluation is uncertain.} Bootstrap 95\% confidence intervals for accuracy span 0--80\%, quantitatively demonstrating that the 5-sample test set provides insufficient statistical power for reliable performance estimation.
